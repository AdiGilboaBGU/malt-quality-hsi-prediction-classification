...
#### CLASSIFICATION per 100 set seeds
############################################################################
...
for target_var in y_df.columns:
    ...
    for seed in seed_values:
        ...
        auc = roc_auc_score(y_test, model.predict_proba(X_test_transformed)[:, 1]) if hasattr(model, "predict_proba") else None
        mv_acc = predict_majority_vote_per_seed(model, X_test_transformed, groups_test, y_test)

        aggregated_results.append([target_var, selected_method, selected_model, acc, f1, auc, mv_acc])

    df_results = pd.DataFrame(aggregated_results, columns=[
        'Y Variable', 'Feature Selection Method', 'Model', 'Accuracy', 'F1 Score', 'AUC', 'Majority Vote Accuracy'])
    ...
    stats_df = df_results.groupby(['Y Variable', 'Feature Selection Method', 'Model']).agg({
        ...
        'Majority Vote Accuracy': ['mean', 'std', 'min', 'max']
    }).reset_index()
    ...
    best_model = stats_df.loc[stats_df.groupby("Y Variable")['Majority Vote Accuracy mean'].idxmax().values]
    print(f"\nBest model for {target_var}:")
    print(best_model.to_string(index=False))
    df_all.append(best_model)

# Save
df_best = pd.concat(df_all, ignore_index=True)
df_best.to_excel(r'G:\My Drive\Thesis\Temp_Work\excel_files_final\ML_results\Best_Pipline_Per_Y\Classification_Per_Seed\Best_Pipeline_Per_Seed_Per_Image_100_Set_Seed.xlsx', index=False)
