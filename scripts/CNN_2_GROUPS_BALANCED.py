...
def plot_history(history, param_name):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(fr'G:\My Drive\Thesis\Temp_Work\excel_files_final\CNN_results\results_plots_groups_balanced_2_classes\{param_name}_training_validation_loss.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(fr'G:\My Drive\Thesis\Temp_Work\excel_files_final\CNN_results\results_plots_groups_balanced_2_classes\{param_name}_training_validation_accuracy.png')
    plt.close()
...
def plot_density_and_roc(X_test, y_test, model, param_name):
    ...
    plt.ylabel('Density')
    plt.legend()
    plt.savefig(fr'G:\My Drive\Thesis\Temp_Work\excel_files_final\CNN_results\results_plots_groups_balanced_2_classes\{param_name}_density_plot.png')
    plt.close()
    ...
    plt.grid(True)
    plt.savefig(fr'G:\My Drive\Thesis\Temp_Work\excel_files_final\CNN_results\results_plots_groups_balanced_2_classes\{param_name}_roc_curve_plot.png')
    plt.close()
...
def evaluate_model(model, X, y, dataset_name, param_name):
    ...
    # Evaluate the model
    results = model.evaluate(X, y, verbose=0)  # results now contain loss and additional metrics, assuming AUC is set as a metric during model compilation
...
def save_results_all_in_one(train_metrics, train_confusion_mtx, y_train, 
    ...
    file_path = fr'G:\My Drive\Thesis\Temp_Work\excel_files_final\CNN_results\results_2_classes_groups_balanced_seed_77\results2_{param_name}.xlsx'
...
def split_groups(groups_list):
    """Splits a list of groups into Train (80%), Test (10%), and Validation (10%)."""
    train_groups, temp_groups = train_test_split(groups_list, test_size=0.25, random_state=77)  # 80% Train
    val_groups, test_groups = train_test_split(temp_groups, test_size=0.4, random_state=77)  # 10% Val, 10% Test
...
for column in y_labels_df.columns:
    ...
    group_class_map = {g: y[groups == g][0] for g in np.unique(groups)}  # Map each group to its label
    ...
    train_groups_0, val_groups_0, test_groups_0 = split_groups(groups_0)
    train_groups_1, val_groups_1, test_groups_1 = split_groups(groups_1)

    # Merge groups of both classes
    train_groups = np.array(train_groups_0 + train_groups_1)
    val_groups = np.array(val_groups_0 + val_groups_1)
    ...
    # === Step 4: Verify class balance in each split ===
    ...
    # === Step 6: Verify no group overlap ===
    common_train_val = set(groups_train) & set(groups_val)
    common_train_test = set(groups_train) & set(groups_test)
    common_val_test = set(groups_val) & set(groups_test)
    
    print(f"Groups shared between Train and Validation: {common_train_val}")
    print(f"Groups shared between Train and Test: {common_train_test}")
    print(f"Groups shared between Validation and Test: {common_val_test}")
